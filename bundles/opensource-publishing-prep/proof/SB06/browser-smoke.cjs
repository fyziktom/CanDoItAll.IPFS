...
const baseUrl = process.env.SB06_BASE_URL || "http://127.0.0.1:5093";
...
async function captureRoute(context, viewport, route) {
  ...
  const screenshotPath = path.join(screenshotDir, `sb06-${route.name}-${viewport.name}.png`);
...
async function captureShareModal(context, viewport) {
  ...
  const screenshotPath = path.join(screenshotDir, `sb06-remote-pin-share-modal-${viewport.name}.png`);
  await page.screenshot({ path: screenshotPath, fullPage: false });
  summary.screenshots.push({
    label,
    route: "/files",
...
async function seedPinnedFile(page) {
  await page.evaluate(async () => {
    ...
    form.append(
      "files",
      new File(["SB06 remote pin share modal seed"], "sb06-share-modal-seed.txt", { type: "text/plain" }));
