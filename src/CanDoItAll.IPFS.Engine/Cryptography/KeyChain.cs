...
namespace Ipfs.Engine.Cryptography
{
    ...
    public partial class KeyChain : Ipfs.CoreApi.IKeyApi
    {
        ...
        /// <inheritdoc />
        public Task<IKey> CreateAsync(string name, string keyType, int size, CancellationToken cancel = default)
        {
            return GeneratePrivateKeyAsync(name, keyType, size, cancel);
        }
        ...
        private static MultiHash CreateKeyId(AsymmetricKeyParameter key)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                var alg = (ms.Length <= 48) ? "identity" : MultiHash.DefaultAlgorithmName;
