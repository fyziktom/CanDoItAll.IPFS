...
namespace Ipfs.Engine
{
    ...
    public class FileStoreTest
    {
        ...
        FileStore<int, Entity> Store
        {
            get
            {
                return new FileStore<int, Entity>(
                    TestFixture.Ipfs.Options.Repository.Folder,
                    $"test-filestore/{TestContext.TestName}")
                {
                    KeyToFileName = key => key.ToString(),
                    FileNameToKey = name => int.Parse(name)
                };
            }
        }
        ...
        {
            var folder = Path.Combine(TestFixture.Ipfs.Options.Repository.Folder, "test-filestore", TestContext.TestName);
        ...
        public async Task Length()
        {
            ...
            var length = await store.SizeOfAsync(6);
            Assert.IsTrue(length.HasValue);
            Assert.AreNotEqual(0UL, length.Value);
        }
        ...
        {
            var store = Store;
            var length = await store.SizeOfAsync(7);
